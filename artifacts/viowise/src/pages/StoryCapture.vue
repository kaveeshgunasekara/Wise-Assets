  ...
  <div class="min-h-screen bg-pattern flex flex-col">
    ...
    <main class="flex-1 max-w-3xl mx-auto w-full px-6 py-12">
      ...
      <div class="bg-[#F4F1FC] border border-[#C5BCDF] p-8 sm:p-12 rounded-[16px] card-shadow mb-10 relative">
        ...
        <h2 class="text-[20px] font-semibold text-foreground mb-4">On starting over</h2>
        ...
        <div class="flex items-center justify-between">
          <div class="font-medium text-[16px] text-foreground/80">— Grace, 72</div>
      ...
      <template v-else>
        <div class="flex flex-col sm:flex-row gap-4 mb-6">
          ...
          <button @click="handleShare" class="flex-1 h-[56px] bg-primary text-white rounded-[12px] text-[18px] font-medium hover:bg-primary-hover transition-colors shadow-lg">
      ...
      <div class="flex flex-col items-center gap-6 pt-12 border-t border-border">
        ...
        <button v-else @click="scheduleMessage = true" class="text-primary font-medium text-[18px] hover:underline px-6 py-3 rounded-xl hover:bg-primary/5 transition">
...
const originalQuoteText = "It's never too late to begin again. I re-took my nursing exams at 38. It was hard, but it reminded me that courage grows with each small step.";
const quoteText = ref(originalQuoteText);

const name = computed(() => store.role === "mentor" ? "Grace" : "Sam");
...
function handleShare() {
  ...
  actionMessage.value = "Request sent to Sam for approval.";
}
