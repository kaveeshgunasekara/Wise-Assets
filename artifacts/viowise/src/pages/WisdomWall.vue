<template>
  <div class="min-h-screen bg-pattern flex flex-col">
    ...
    <div class="bg-secondary p-2 text-center text-base font-medium border-b border-border z-10 relative">
      Demo: Viewing as {{ store.role === 'mentor' ? 'Grace (Mentor)' : 'Sam (Learner)' }}
      <button @click="store.setRole(store.role === 'mentor' ? 'learner' : 'mentor')" class="ml-4 text-primary underline">Switch role</button>
    </div>

    <main class="flex-1 max-w-5xl mx-auto w-full px-6 py-8">

      <!-- Learner Approval Card -->
      <div v-if="store.role === 'learner' && store.pendingStory && tab === 'For you'" class="bg-[#F4F1FC] border border-[#C5BCDF] rounded-[16px] p-6 mb-8 flex flex-col sm:flex-row gap-6 items-start sm:items-center justify-between">
        <div>
      ...
      <!-- Tabs -->
      <div class="flex gap-2 border-b border-border mb-8 overflow-x-auto pb-1">
        <button
          v-for="t in tabs"
          :key="t"
          @click="tab = t"
          :class="['px-4 py-3 text-[18px] font-medium whitespace-nowrap relative', tab === t ? 'text-primary' : 'text-foreground/60 hover:text-foreground']"
        >
          {{ t }}
          <span v-if="t === 'Requests' && requests.length > 0" class="ml-2 bg-primary text-white text-base px-2 py-0.5 rounded-full">{{ requests.length }}</span>
          <div v-if="tab === t" class="absolute bottom-0 left-0 right-0 h-[3px] bg-primary rounded-t-full" />
        </button>
      </div>

      <!-- Requests Tab -->
      <div v-if="tab === 'Requests' && store.role === 'mentor'" class="space-y-4">
        <div v-for="req in requests" :key="req.id" class="bg-white p-6 rounded-[16px] card-shadow flex flex-col sm:flex-row gap-4 justify-between items-center">
          ...
          <div v-else class="flex gap-3">
            <button @click="requestAction[req.id] = 'Request declined'" class="px-6 py-3 border border-border rounded-[12px] font-medium hover:bg-secondary">Decline</button>
        ...
        <p v-if="requests.length === 0" class="text-center text-foreground/60 py-12">No pending requests.</p>
      </div>

      <!-- My Posts Empty -->
      <div v-else-if="tab === 'My posts' && filteredStories.length === 0" class="text-center py-20 bg-white rounded-[16px] border border-border border-dashed">
      ...
      <template v-else>
        ...
        <div class="flex flex-col md:flex-row gap-4 justify-between mb-8">
          <div class="flex gap-2 overflow-x-auto pb-2 flex-1">
            <button
              ...
              :class="['px-4 py-2 rounded-full border text-[16px] whitespace-nowrap transition-colors', topicFilter === t ? 'bg-primary/10 border-primary text-primary' : 'border-border bg-white text-foreground/70 hover:border-primary/50']"
            >
              {{ t }}
            </button>
          </div>
          <div class="relative w-full md:w-64 shrink-0">
            <input
              ...
              v-model="search"
              class="w-full px-4 h-[44px] rounded-[12px] border border-input focus:ring-3 focus:ring-primary/20 outline-none pl-10 bg-white"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div v-for="story in filteredStories" :key="story.id" class="bg-white p-8 rounded-[16px] card-shadow flex flex-col h-full relative">
          ...
          <div v-if="filteredStories.length === 0" class="col-span-1 md:col-span-2 text-center py-20">
            <p class="text-[18px] text-foreground/60">No wisdom found matching your filters.</p>
            <button @click="search = ''; topicFilter = null" class="mt-4 text-primary font-medium">Reset filters</button>
          </div>
        </div>
      </template>
    </main>

    <footer class="py-8 text-center text-foreground/60 text-[16px] border-t border-border bg-[#F7F5FB]">
